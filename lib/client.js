	...
	factory: (require) => {
		...
		function ServerCard({ server, onEdit, onDelete }) {
			...
			return h(
				...
				h("button", { style: BTN_DANGER, onClick: onDelete }, t("delete"))
			);
		}
		//#endregion
		...
		function McpSection(props) {
			...
			const deleteServer = async (server) => {
				...
				if (!window.confirm(t("confirmDelete")(server.serverName))) return;
				setActErr(null);
				const resp = await callApi(() => API.remove(server.id, revision), setActErr);
				if (resp) await finishOp();
			};
